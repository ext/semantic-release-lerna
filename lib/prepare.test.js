...
test("Update package.json dependency when using exact version", async () => {
	const cwd = tempy.directory();
	const npmrc = tempy.file({ name: ".npmrc" });
	await createProject(cwd, "0.0.0");
...
test("Update package.json dependency when using hat", async () => {
	const cwd = tempy.directory();
	const npmrc = tempy.file({ name: ".npmrc" });
	await createProject(cwd, "0.0.0");
	const foo = await createPackage(
	...
	await createPackage(cwd, "bar", "0.0.1", { changed: true });
...
test("Should not update other dependencies", async () => {
	const cwd = tempy.directory();
	const npmrc = tempy.file({ name: ".npmrc" });
	await createProject(cwd, "0.0.0");
	const foo = await createPackage(
...
test("Handle dependencies from root package", async () => {
	const cwd = tempy.directory();
	const npmrc = tempy.file({ name: ".npmrc" });
