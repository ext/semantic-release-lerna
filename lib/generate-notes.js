...
import loadChangelogConfig from "@semantic-release/release-notes-generator/lib/load-changelog-config";
import HOSTS_CONFIG from "@semantic-release/release-notes-generator/lib/hosts-config";
...
export default async function generateNotes(pluginConfig, context) {
	...
	const { issue, commit, referenceActions, issuePrefixes } =
		Object.values(HOSTS_CONFIG).find((conf) => conf.hostname === hostname) || HOSTS_CONFIG;
	...
	return getStream(object(parsedCommits).pipe(writer(changelogContext, writerOpts)));
}
